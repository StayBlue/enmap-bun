...
Mostly, this documentation will be concentrating on the "persistent" version of Enmap Bun - the one where data is saved automatically through `bun:sqlite`.

If you don't want persistence, the only difference is how you initialize the enmap: 

```javascript
import Enmap from 'enmap-bun';
const myEnmap = new Enmap({ inMemory: true });
...
* `sqliteOptions` Optional. An object of [options](https://bun.sh/docs/runtime/sqlite) to pass to the `bun:sqlite` `Database` constructor.
